...
test('search for cypress on google', async ({ page }) => {
  // Navigate to Google
  ...
  // Type "cypress" into the search box
  const searchBox = page.getByRole('combobox', { name: 'Zoek' });
  await searchBox.click();
  await searchBox.fill('cypress');
  await searchBox.press('Enter');
  
  // Verify that the search results page contains "cypress"
  await expect(page).toHaveTitle(/cypress/i);
});
